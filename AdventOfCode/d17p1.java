    ...
    public static void main(String[] args) throws Exception {
        ...
        while ((line = in.readLine()) != null) {
            ...
            for (int j1 = 0; j1 < initwidth; j1++) {
                states.put(i1 + " " + j1 + " " + 0, line.charAt(j1) == '.' ? false : true);
            }
            i1++;
        }
            ...
            for (int x = dims[0][0]; x <= dims[0][1]; x++) {
                for (int y = dims[1][0]; y <= dims[1][1]; y++) {
                    for (int z = dims[2][0]; z <= dims[2][1]; z++) {
                        ...
                        if(newStates.get(k)){
                            if(x < newdims[0][0]) newdims[0][0] = x;
                            if(y < newdims[1][0]) newdims[1][0] = y;
                            if(z < newdims[2][0]) newdims[2][0] = z;
                            if(x > newdims[0][1]) newdims[0][1] = x;
                            if(y > newdims[1][1]) newdims[1][1] = y;
                            if(z > newdims[2][1]) newdims[2][1] = z;
                        }
                    }
                }
            }
