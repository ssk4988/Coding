...
Node<T>* merge(Node<T> *a, Node<T> *b) {
    ...
    Node<T> *res = new Node<T>(*a);
    res->r = merge(res->r, b);
    if((res->l ? res->l->depth : 0) < (res->r ? res->r->depth : 0)) swap(res->l, res->r);
    res->depth = 1 + (res->r ? res->r->depth : 0);
    return res;
}
template<class T>
Node<T>* pop(Node<T> *r) {
    return merge(r->l, r->r);
}
...
int main() {
    ...
    vector ord(1 << n, vector<Node<array<int, 3>>*>(m+1));
    for(int col = m-1; col >= 0; col--) {
        rep(mask, 0, 1 << n) {
            ...
            ord[mask][col] = ord[mask][col+1];
            rep(i, 0, n) {
                if(!((mask >> i)&1)) continue;
                Node<array<int, 3>> *cur = new Node(array<int, 3>{matching[mask ^ (1 << i)][col+1] + grid[i][col], i, col});
                ord[mask][col] = merge(ord[mask][col], cur);
            }
        }
    }
    // rep(mask, 0, 1 << n) {
    ...
    auto calc = [&](int lim, bool build) -> pair<bool, vii> {
        ...
        rep(u, 0, n) {
            for(auto [mask, col, cost] : q[u]) {
                auto cur = ord[mask][col];
                while(cur) {
                    auto [_, r1, c1] = cur->val;
                    cur = pop(cur);
                    if(sz(q[u+1]) >= k) break;
                    // int r1 = o / m, c1 = o % m;
