...
struct mcmf {
	...
	void add_edge(int u, int v, ll cap, ll cost) {
        // cerr << "adding edge " << u << " to " << v << " cap=" << cap << " cost=" << cost << endl;
		int idx = sz(edges);
...
void solve() {
    ...
    if(canbelast[0]) {
        ...
        // cerr << "checking 0 as last " << co+base << " " << sumin << " " << fl << endl;
    }
    {
        bool sub = out[0];
        // cerr << "sub: " << sub << endl;
        ...
        g.add_edge(n+2, sink, 1, 0);
        auto [fl, co] = g.calc(source, sink);
        // cerr << "flow " << fl << " expect " << (sumin + !sub) << " cost " << co << " with base " << base << endl;
        if(fl == sumin + !sub) ans = min(ans, co + base);
    }
