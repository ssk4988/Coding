...
if ( __name__ == "__main__" ) :
    ...
    for ind_opt in range(len(argv)) :
        ...
        if ( argv[ind_opt] == "--only-changes" ) :
            ind_chg = ind_opt
            only_changes = 1
        if ( argv[ind_opt] == "--maxlines" ) :
            ind_maxlines = ind_opt
            max_lines = int(argv[ind_maxlines+1])

    argv = list(argv)
    if ( ind_css >= 0 ) :
        del argv[ind_css:ind_css+2]
    if ( ind_maxlines >= 0 ) :
        del argv[ind_maxlines:ind_maxlines+2]
    if ( ind_chg >= 0 ) :
    ...
    diff_file = file("diff.txt", "r")
    diff_output = diff_file.readlines()
    diff_file.close()
    ...
    if ( len(changed) == 0 ) :
        ...
        keys = changed.keys()
        keys.sort()
        if len(keys) > MAX_LINKS:
        	count = MAX_LINKS
        else:
        	count = len(keys)
        for i in range(0, count):
            changed_lnks += "<a href=\"#F1_%d\">%d</a>, " % (keys[i], keys[i])
    ...
    if ( len(added) == 0 ) :
        ...
        if len(keys) > MAX_LINKS:
        	...
        	count = len(keys)
        for i in range(0, count):
            added_lnks += "<a href=\"#F2_%d\">%d</a>, " % (keys[i], keys[i])
    ...
    if ( len(deleted) == 0 ) :
        ...
        if len(keys) > MAX_LINKS:
        	...
        	count = len(keys)
        for i in range(0, count):
            deleted_lnks += "<a href=\"#F1_%d\">%d</a>, " % (keys[i], keys[i])
...
""" % (changed_lnks, added_lnks, deleted_lnks,
    ...
    nl1 = nl2 = totalLines = 0
    while not ( (nl1 >= len(f1_lines)) and (nl2 >= len(f2_lines)) ) and (max_lines == 0 or totalLines < max_lines) :
    	totalLines += 1
...
""" % (nl1+1, nl1+1, str2html(f1_lines[nl1]))
    ...
    <tr>
        <td class="linenum"><a name="F1_%d">%d</a></td>
        <td class="modified"><pre>%s</pre></td>
        <td width="16">&nbsp;</td>
