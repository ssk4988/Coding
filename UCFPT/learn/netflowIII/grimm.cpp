...
int main()
{
    ...
    while (true)
    {
        ...
        for (ll i = l; i <= r; i++)
        {
            ...
            factorsLeft[factorsLeft.size() - 1] = factors.back().size();
        ...
        {
            int bitsLeft = 55;
            while (rf < firstF && bitsLeft >= factors[rf].size())
            {
                bitsLeft -= factors[rf].size();
                rf++;
            }
            MCMF g(cnt);
            bitsLeft = 56;
            for (int i = lf; i < firstF; i++)
            {
                ...
                for (auto it = factors[i].rbegin(); it != factors[i].rend(); it++)
                {
        ...
        {
            cout << matching[i] << (i + 1 < matching.size() ? " " : nL);
        }
    }
