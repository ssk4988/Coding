...
pii solve(vi grid, int n, int m) {
    ...
    int tog = 0;
    rep(i, 1, n) {
        tog += __builtin_popcount(grid[i-1]);
        if(i+1 < n) grid[i+1] ^= grid[i-1];
        grid[i] ^= (grid[i-1] >> 1) ^ ((grid[i-1] << 1) & allmask);
        // rep(j, 0, m) {
        //     if((grid[i-1] >> j)&1) {
        //         grid[i-1] ^= 1 << j;
        //         tog++;
...
int main()
{
    ...
    rep(cn, 0, nc) {
        ...
        auto [base, tog2] = solve(grid, r, c);
        vi changes(c);
        rep(i, 0, c) {
            if(i) grid[0] ^= 1 << (i-1);
            if(i+1<c) grid[0] ^= 1 << (i+1);
            if(r >= 2) grid[1] ^= 1 << i;
            changes[i] = solve(grid, r, c).first ^ base;
            if(i) grid[0] ^= 1 << (i-1);
            if(i+1<c) grid[0] ^= 1 << (i+1);
            if(r >= 2) grid[1] ^= 1 << i;
        }
        ...
        rep(mask, 0, 1 << c) {
            ...
            if(cur == 0 && !seen) {
                vi grid2(grid);
                grid2[0] ^= (mask >> 1) ^ ((mask << 1) & allmask);
                if(r >= 2) grid2[1] ^= mask;
                auto [last, tog] = solve(grid2, r, c);
                ans = min(ans, {bits + tog, mask});
            }
        }
