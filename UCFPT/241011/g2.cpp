...
int main()
{
    ...
    // xm.max_load_factor(5);
    xm.reserve(n);

    // cout << xm.max_load_factor() << endl;
    rep(i, 0, n){
        cin >> pts[i].x >> pts[i].y;
    ...
    while(sz(qu)) {
        ...
        for(auto idx : lines) {
            ...
            if(x1 <= pts[L].x && pts[R-1].x <= x2) {
                ...
                for(int i : {i1, i2}) {
                    if(i == -1) continue;
                    if(res[idx] == -1 && aboveLine(x1, y1, x2, y2, hull[i].x, hull[i].y)) res[idx] = xm[hull[i].x].f;
                    if(res[idx] == -1 && aboveLine(x1, y1, x2, y2, hull[(i+1)%sz(hull)].x, hull[(i+1)%sz(hull)].y)) res[idx] = xm[hull[(i+1)%sz(hull)].x].f;
                }
                continue;
            }
            left.pb(idx), right.pb(idx);
        }
