...
int main()
{
    ...
    rep(i, 0, n){
        ...
        rep(j, i + 1, n){
            if(da[i] != da[j]){
                ll inter = (a[i] - a[j]) / (da[j] - da[i]);
                int s1 = ((db[i] - da[i]) * inter + (b[i] - a[i])) < 0, s2 = ((db[j] - da[j]) * inter + (b[j] - a[j])) < 0;
                if(!s1 && !s2){
                    if(inter >= 0 && inter <= m) crossover.insert(inter);
                    if(inter + 1 >= 0 && inter + 1 <= m) crossover.insert(inter + 1);
                    if(inter - 1 >= 0 && inter - 1 <= m) crossover.insert(inter - 1);
            ...
            // vl pts;
            // ll inter1 = (a[i] - b[i]) / (db[i] - da[i]), inter2 = (a[j] - b[j]) / (db[j] - da[j]);
            // pts.pb(inter1), pts.pb(inter1 - 1), pts.pb(inter1 + 1);
            // pts.pb(inter2), pts.pb(inter2 - 1), pts.pb(inter2 + 1);
            // for(ll inter : pts){
            //     if(inter < 0 || inter >= m) continue;
            //     int s1 = ((db[i] - da[i]) * inter + (b[i] - a[i])) < 0, s2 = ((db[j] - da[j]) * inter + (b[j] - a[j])) < 0;
            //     if(s1 != s2) continue;
            //     if(!s1){
