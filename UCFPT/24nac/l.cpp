...
struct Shape {
	...
	// quad goes like ang[0], side[0], ang[1], side[1], ang[2], side[2], ang[3], side[3]s
	Shape() {}
		...
		rep(bot, 0, 4) {
			for (auto b : tos[bot]) {
				if (sideeq(t.sides[0], b.sides[0]) && angeq(t.angles[1], b.angles[1]) && sidelt(b.sides[1], t.sides[1])) {
					...
					rep(top, 0, 4) {
						if (bot == top) continue;
						if (other == tris[top]) res.pb((1 << bot) | (1 << top));
					}
				}
			}
		}
...
vi quad2(Quad q) {
	...
	rep(_, 0, 2) {
		if (anglt(q.angles[1], pi)) {
			...
			auto t = sas(q.sides[2], q.angles[3], q.sides[3]);
			rep(bot, 0, 4) {
				if (!(b == tris[bot])) continue;
				rep(top, 0, 4) {
					if (bot == top || !(t == tris[top])) continue;
					res.pb((1 << top) | (1 << bot));
				}
			}
		}
...
vi quint3(Quint q) {
	...
	rep(_, 0, 2) {
		...
		q = q.rev();
		while (!anglt(pi, q.angles[1])) q = q.rot();
	}
	return res;
}
...
vi quad4(Quad q)
{
	...
	rep(bot, 0, 4) {
		if (tris[bot] == half) {
			for (int cand : ores) {
				if ((cand & (1 << bot)) == 0) res.pb(cand | (1 << bot));
			}
		}
	}
