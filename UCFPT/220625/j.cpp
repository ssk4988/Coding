...
int prevPath[N]; // array for memorizing alternating paths
...
void augment() // main function of the algorithm
{
    ...
    memset(prevPath, -1, sizeof(prevPath)); // init set prev - for the alternating tree
    ...
    if (y < n) // we found augmenting path!
    {
        ...
        for (int cx = x, cy = y, ty; cx != -2; cx = prevPath[cx], cy = ty)
        {
            ty = xy[cx];
            yx[cy] = cx;
            xy[cx] = cy;
...
{
    S[x] = true;                // add x to S
    prevPath[x] = prevx;            // we need this when augmenting
