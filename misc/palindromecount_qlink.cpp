...
struct Eertree {
    vi slink = {0, 0}, len = {-1, 0}, qlink = {0, 0};
    vvi down;
    int cur = 0, t = 0;
    vi curst, tst, nodes;
    vector<array<int, 3>> downst;
    string str;
    Eertree() : down(2, vi(26, -1)) {}
    void add(char c) {
        ...
        int ci = c - 'a';
        while (t <= 0 || str[t-1] != c) {
            if(str[(sz(str)-1) - len[slink[cur]]-1] == c) t = (sz(str)-1) - len[cur = slink[cur]];
            else t = (sz(str)-1) - len[cur = qlink[cur]];
        }
        if (down[cur][ci] == -1) {
            ...
            if (len.back() > 1) {
                ...
                // while(t <= 0 || str[t-1] != c);
                // t = (sz(str)-1) - len[cur = slink[cur]];
                do {
                    if(str[(sz(str)-1) - len[slink[cur]]-1] == c) t = (sz(str)-1) - len[cur = slink[cur]];
                    else t = (sz(str)-1) - len[cur = qlink[cur]];
                }
                while (t <= 0 || str[t-1] != c);
                // while(t <= 0 || str[t-1] != c) t = (sz(str)-1) - len[cur = qlink[cur]];
                slink.push_back(down[cur][ci]);
            } else slink.push_back(1);
            cur = sz(slink) - 1;
            if (len[slink[cur]] <= 0) qlink.push_back(slink[cur]);
            else
            if (str[sz(str)-1 - len[slink[cur]]] == str[sz(str)-1 - len[slink[slink[cur]]]]) qlink.push_back(qlink[slink[cur]]);
            else qlink.push_back(slink[slink[cur]]);
