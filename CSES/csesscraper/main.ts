...
async function parseStatPage(link: string) {
    ...
    users.forEach((id, i) => {
        if (!(id in IDToUsers)) return;
        const username = IDToUsers[id];
        userList.push(
            `${username} - Rank ${(i % 5) + 1} in ${i >= 5 ? 'Shortest Code' : 'Fastest Code'}`
        );
    });
    return userList;
}
...
async function parseProblems() {
    ...
    for (const problem of problems) {
        const link = 'https://cses.fi' + (problem.children[0] as HTMLAnchorElement).href;
        const name = (problem.children[0] as HTMLAnchorElement).innerHTML;
