...
int main()
{
    ...
    rep(cn, 0, nc) {
        ...
        int t = 0;
        while(true) {
            if(minwait[n-1] != inf) break;
            vi newminwait(minwait);
            rep(u, 0, n) {
                if(minwait[u] == inf) continue;
                int v = adj[u][t % sz(adj[u])];
                newminwait[v] = min(newminwait[v], minwait[u]-1);
            }
            minwait = newminwait;
            t++;
        }
        cout << t << " " << t+minwait[n-1] << "\n";
