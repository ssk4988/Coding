...
int main() {
    ...
    // vvi times(n);
    // rep(i, 0, k){
    //     // cout << i << " " << states[i].first << " " << states[i].second << "\n";
    // }
    for(auto [a, b, c, d] : ed){
    ...
    for(auto &[key, val] : ts){
        ...
        for(int id : val){
            ...
            score[id] = curscore;
            // cout << id << " gets " << curscore << "\n";
            if(airport == n-1){
                // cout << n-1 << " airport gets " << curscore << "\n";
