...
int main()
{
    ...
    rep(i, 2, isPrime.size()){
        if(isPrime[i]){
            for(int j = 2 * i; j < sz(isPrime); j+=i){
                isPrime[j] = false;
            }
            prime.pb(i);
            // if(DEBUG) cout << i << nL;
    ...
    vector<vl> dp(n + 1, vl(1 + rel.size(), -1));
    vector<vector<ld>> dec(n + 1, vector<ld>(1 + rel.size()));
    // first dim is num used, 2nd is which ind
    // dp[0][0] = 1;
    // dec[0][0] = 1;
    auto solve = [&](int used, int idx, auto &&solve) -> ll {
        if(used == 0 && idx == -1) return 1;
        if(used < 0 || idx < 0) return 0;
        ll &ans = dp[used][idx];
        if(ans != -1) return ans;
        ans = 0;
        ans = mod(ans + mod(solve(used - 1, idx - 1, solve) * factinv[freqr[idx] - 1]));
        ans = mod(ans + mod(solve(used, idx - 1, solve) * factinv[freqr[idx]]));
        return ans;
    };
    ...
    // cout << (rel.size() >= n ? mod(mod(dp[n].back() * fact[n]) * denom1) : 0) << nL;
    cout << mod(mod(solve(n, rel.size() - 1, solve) * fact[n]) * denom1) << nL;
